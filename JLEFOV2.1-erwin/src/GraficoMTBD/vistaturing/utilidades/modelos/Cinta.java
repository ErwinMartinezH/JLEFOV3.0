...
public final class Cinta implements Iterable<Simbolo> {
    ...
    public void setSimbolo(Simbolo simbolo, MT.Direccion direccion) {
        ...
        // MT Bidireccional
        while (_pos < 0) {
            _simbolos.addFirst(Simbolo.Blanco);
            ++_startPos;
            ++_pos;
        }
    ...
    public Simbolo getSimbolo() {
        ...
        if (_pos < 0) return Simbolo.Blanco;
    ...
    public Simbolo simboloAt(int index) {
        ...
        var pos = index + _startPos;

        if (pos < 0) return Simbolo.Blanco;
        if (pos >= length()) return Simbolo.Blanco;

        return _simbolos.get(pos);
    }
